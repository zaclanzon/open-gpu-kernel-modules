static NvBool TestAllocation(NvU32 windowCount, NvBool reuse, NvU32 scalerTiles, NvU32 scalingMask,
                             NvU32 overlayMask, NvU32 tileChoices)
{
    NVDevEvoRec dev = {0};
    ...
    for (NvU32 head = 0; head < 4; head++) {
        ...
        if (reuse) {
            ...
            input.head[head].multiTileConfig.phywinsMask[0] = NVBIT(windowCount - 8 + 2 * head);
            input.head[head].multiTileConfig.phywinsMask[1] = NVBIT(windowCount - 8 + 2 * head + 1);
        }
        if (scalingMask & NVBIT(head)) {
            requiredScalerTiles += required[head];
        }
        totalTiles += required[head];
        totalWindows += required[head] * (1 + usage[head].layer[1].usable);
    }

    const NVEvoIsModePossibleDispInput originalInput = input;
    NvBool result = EvoAssignHwHeadMultiTileConfigDispOutputCA(&disp, &input, required, &output);
    assert(memcmp(&input, &originalInput, sizeof(input)) == 0);
    NvBool expected =
        totalTiles <= 8 && totalWindows <= windowCount && requiredScalerTiles <= scalerTiles;
    if (result != expected) {
        printf("FAIL: windows=%u reuse=%u scalerTiles=%u scaling=%u "
               "overlays=%u tiles=%u "
               "needTiles=%u needWindows=%u accepted=%d\n",
               windowCount, reuse, scalerTiles, scalingMask, overlayMask, tileChoices, totalTiles,
               totalWindows, result);
        return FALSE;
    }
    if (result) {
        for (NvU32 head = 0; head < 4; head++) {
            const NVHwHeadMultiTileConfigRec *pConfig = &output.head[head].multiTileConfig;
            ...
            for (NvU32 layer = 0; layer < 2; layer++) {
                NvU32 expectedCount = usage[head].layer[layer].usable ? required[head] : 0;
                assert(nvPopCount32(pConfig->phywinsMask[layer]) == expectedCount);
                assert(!(pConfig->phywinsMask[layer] & ~(NvU32)(NVBIT64(windowCount) - 1)));
...
static void TestBoundaryConditions(void)
{
    ...
    assert(EvoAssignHwHeadMultiTileConfigDispOutputCA(&disp, &input, required, &output));
            ...
            for (NvU32 scalerTiles = 0; scalerTiles <= 8; scalerTiles++) {
                for (NvU32 scalingMask = 0; scalingMask < 16; scalingMask++) {
                    for (NvU32 overlayMask = 0; overlayMask < 16; overlayMask++) {
                        for (NvU32 tileChoices = 0; tileChoices < 256; tileChoices++) {
                            assert(TestAllocation(windowCount, reuse, scalerTiles, scalingMask,
